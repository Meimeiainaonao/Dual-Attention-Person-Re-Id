...
def train_test_Sequence(model, dataset, trainInds, testInds, nEpochs,sampleSeqLength,lr,momentum,samplingEpochs,model2):
	nTrainPersons = len(trainInds)	# 150
	Sigmoid=nn.Sigmoid()
	
	classifierLayer = nn.Linear(128,150)
	...
	optimizer = torch.optim.SGD(list(model.parameters())+list(model2.parameters()), lr=lr, momentum=momentum) 
	...
	for eph in range(1, nEpochs+1): 
		...
		for i in xrange((nTrainPersons*2)):
			...
			#choose a positive pair, both sequences show the same person
				pushPull = 1
		        	camA = 0											# Query Cam ( Probe Cam )
		        	camB = 1											# Candidate Cam
			...
			#ipdb.set_trace()
			cnn_1,feature_1_1, feature_2_1=model(netInputA)
			#ipdb.set_trace()
			cnn_2,feature_1_2, feature_2_2=model(netInputB)
			feature_vec1,feature_vec2=model2(cnn_1,feature_1_1, feature_2_1,cnn_2,feature_1_2, feature_2_2)
#########################################################################################
			
			#ipdb.set_trace()
			classifier_1 = classifierLayer(feature_vec1).cpu()   
			classifier_2 = classifierLayer(feature_vec2).cpu()
			
			logsoftmax1 = logsoftmax(classifier_1).cuda() 
			logsoftmax2 = logsoftmax(classifier_2).cuda()          
			...
			dist=distance(feature_vec1,feature_vec2).cuda()
			#ipdb.set_trace()
		...
		if (eph % samplingEpochs == 0):
			#ipdb.set_trace()
			cmcTest,simMatTest = computeCMC_MeanPool_RNN(dataset,testInds,model,sampleSeqLength,model2)
			cmcTrain,simMatTrain = computeCMC_MeanPool_RNN(dataset,trainInds,model,sampleSeqLength,model2)
