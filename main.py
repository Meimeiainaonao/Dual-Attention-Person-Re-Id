...
from utils import progress_bar												# not used though , can be ignored
from torch.autograd import Variable
from prepareDataset import prepareDataset
from datasetUtils import partitionDataset,feature_extraction_using_vgg
from build_model import build_model
from build_model2 import build_model2
...
parser.add_argument('--nEpochs', default=2000, type=int, help='number of training epochs')
parser.add_argument('--gpu', default=0, type=int, help='Which gpu should use')
...
# path of the dataset
if args.dataset==1:
    seqRootRGB = '../pytorch-FCN-FINAL-(ilids)/data/i-LIDS-VID/i-LIDS-VID/sequences/'
    seqRootOF = '../pytorch-FCN-FINAL-(ilids)/data/i-LIDS-VID-OF-HVP/sequences/'
else:
...
"""
#train the model
model,model2=train_test_Sequence(fcn_attention_model, dataset, trainInds, testInds, args.nEpochs, args.sampleSeqLength, args.lr,args.momentum, args.samplingEpochs,fcn_extended)

#save trained model
torch.save(model, 'trained_model/model1.pt')
torch.save(model2, 'trained_model1/model2.pt')
#load trained model
...
for n in xrange(len(nTestImages)):
	...
	computeCMC_MeanPool_RNN(dataset,testInds,model,nTestImages[n],model2)
